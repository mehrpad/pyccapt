...
from pyccapt.calibration.core.peak_models import (
    _expand_mrp_window, _fit_asymmetric_mrp, _fit_gaussian_mrp,
    _fit_voigt_mrp, _format_mrp_value, _mrp_sides_from_values,
    _mrp_width_from_fwhm, _sanitize_tail_widths, _select_peak_index,
)
...
def _fast_mrp_core(data, x1, x2, bin_size):
    ...
    # Bin over the in-window DATA extent. NOTE: a 2026-06 audit briefly changed
    # this to bin over the requested [x1, x2] so the realized bin width would
    # match bin_size. That shifted the MRP score (~20% on real peaks), which
    # feeds the adaptive-residual candidate SELECTION, and reintroduced a left
    # shoulder on voltage/bowl-calibrated peaks. Reverted: keep the long-standing
    # data-extent binning the calibration was tuned against.

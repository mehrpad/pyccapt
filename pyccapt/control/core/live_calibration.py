...
def apply_corrections(
	...
	if mode == 'tof':
		...
		mc_corr = tof2mc_simple.tof_2_mc(
			t_corr, params.t_0, v, x_cm, y_cm,
...
def _fit_bowl_polynomial(
	...
	samples = _collect_polar_samples(
		x_mm, y_mm, t_or_mc, v, maximum_location, sample_size_mm, det_diam_mm,
	)
	if samples["x"].size < 6:
		return None
	try:
...
class LiveCalibrationWorker(QtCore.QThread):
	...
	def run(self) -> None:
		_log.info(
			"Live calibration worker started (mode=%s, refit %.1f s, "
			"min %d events, min R²=%.2f, EMA=%d)",
			self._mode, self._refit_interval_s, self._min_events,
			self._min_fit_quality, self._ema_window_size,
		)
	...
	def _refit_once(self) -> Optional[CalibrationParams]:
		...
		if t_ns.size < self._min_events:
			self.status_changed.emit(
				f"calibrating… ({t_ns.size}/{self._min_events} events)"
			)
			return None

		t_0 = float(self._conf.get(
	...
	def _refit_tof(self, t, v, x_cm, y_cm, x_mm, y_mm, t_0, L_mm, det_diam_mm):
		...
		mc1 = tof2mc_simple.tof_2_mc(
			t1, t_0, v, x_cm, y_cm, flightPathLength=L_mm,
		)
		...
		vol_fit, r2 = _fit_voltage_polynomial(
			v_peak, t_peak, maximum_location_t, self._volt_sample_size,
		)
		if vol_fit is None or r2 < self._min_fit_quality:
			self.status_changed.emit(
				f"voltage fit weak (R²={r2:.2f}); keeping previous params"
			)
			return None
		...
		bowl_fit = _fit_bowl_polynomial(
			x_mm_peak, y_mm_peak, t_v_peak, v_peak,
			maximum_location_t_v, self._bowl_sample_size_mm, det_diam_mm,
		)
		if bowl_fit is None:
		...
		self.status_changed.emit(
			f"tof: peak {peak_mass:.2f} Da → {target:.2f} Da, "
			f"R²={r2:.2f}, n={t.size}"
		)
		return params
	...
	def _refit_mc(self, t, v, x_cm, y_cm, x_mm, y_mm, t_0, L_mm, det_diam_mm):
		# Step 0: raw MC.
		mc = tof2mc_simple.tof_2_mc(
			t, t_0, v, x_cm, y_cm, flightPathLength=L_mm,
		)
		...
		bowl_init = _fit_bowl_polynomial(
			x_mm_peak, y_mm_peak, mc_peak, v_peak,
			maximum_location_mc, self._bowl_sample_size_mm, det_diam_mm,
		)
		...
		vol_fit, r2 = _fit_voltage_polynomial(
			v_peak, mc1_peak, maximum_location_mc1, self._volt_sample_size,
		)
		if vol_fit is None or r2 < self._min_fit_quality:
			self.status_changed.emit(
				f"voltage fit weak (R²={r2:.2f}); keeping previous params"
		...
		bowl_final = _fit_bowl_polynomial(
			x_mm_peak, y_mm_peak, mc2_peak, v_peak,
			maximum_location_mc2, self._bowl_sample_size_mm, det_diam_mm,
		)
		...
		self.status_changed.emit(
			f"mc: peak {peak_mass:.2f} Da → {target:.2f} Da, "
			f"R²={r2:.2f}, n={t.size}"
		)
		return params

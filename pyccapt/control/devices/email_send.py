	...
	if toml_path.is_file():
		if tomllib is None:
			raise EmailNotConfigured(
				f"Found {toml_path} but no TOML parser is available. "
		...
		if not password or not sender:
			raise EmailNotConfigured(
				f"{toml_path} is missing 'sender_email' or 'password'. "
				"Compare with email_credentials.example.toml."
			)
		return {
...
def send_email(email: str, subject: str, message: str, variables=None) -> list[str]:
	...
	msg, all_recipients, attached = _build_message(
		recipient, subject, message, config, variables=variables
	)

...
class Ui_Stage_Control(object):
    ...
    def setupUi(self, Stage_Control):
        ...
        self.stage_stop.setStyleSheet("QPushButton{background: rgb(220,80,80); color: white; font-weight: bold;}")
    ...
    def _confirm_reference_no_sample(self):
        ...
        warning.setStyleSheet("QLabel{color:#c00000;}QMessageBox{background-color:#fff5f5;}")
        warning.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No)
        warning.setDefaultButton(QtWidgets.QMessageBox.StandardButton.No)
    ...
    def _set_error(self, message):
        if not message:
            self.Error.setText("")
            return
        self.Error.setText(f'<html><body><p style="color:#c00000;">{message}</p></body></html>')

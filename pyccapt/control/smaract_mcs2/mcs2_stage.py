...
class SmarActStage:
    ...
    def __init__(self, locator: str):
        if ctl is None:
            raise SmarActStageError(f"smaract.ctl SDK is not available: {_IMPORT_ERROR}")
        if not locator:
        ...
        try:
            ...
            raise SmarActStageError(
                f"ctl.Open('{locator}') failed (error code {code:#06x})." + (f"\n  Hint: {hint}" if hint else "")
            ) from exc
    ...
    def _wait(self, channel: int, timeout_s: float = 30.0, cancel_event: "Optional[Any]" = None) -> None:
        ...
        while time.time() < deadline:
            ...
            if not (state & ctl.ChannelState.ACTIVELY_MOVING):
                return
            time.sleep(0.02)
        raise SmarActStageError(f"Channel {channel} did not stop within {timeout_s} s.")

...
def select_peak(v: Variables, mode: str, prominence: int = 100, distance: int = 500) -> dict:
    ...
    peaks = calibration.auto_detect_reference_peaks(
        arr, n_peaks=1, prominence=prominence, distance=distance, hist_bin_size=hist_bin
    )
    p = peaks[0]
...
def evaluate_spectrum(arr: np.ndarray, mode: str) -> dict:
    ...
    return {
        ...
        "dominant_mrp50": round(dominant_mrp50, 2) if np.isfinite(dominant_mrp50) else None,
        "peaks": details,
    }
...
def _run_adaptive_residual(
    ...
    return adaptive_residual_calibration(
        ...
        n_windows=n_windows,
        overlap=0.5,
        template_bin_size=template_bin_size,
...
def _run_ml(v, cal_mode, n_peaks=6, prominence=100, distance=10, n_estimators=200, max_depth=5):
...
def define_pipelines(flight_path_mm, det_diam):
    ...
    pipelines.append(
        (
            "initial",
            {"init": "standard"},
            lambda v, m: _run_initial_mc(v, det_diam) if m == "mc" else _run_initial_tof(v, flight_path_mm, det_diam),
        )
    )
...
def main():
    ...
    with open(args.output_json, "w") as f:
        json.dump(
            {"dataset": args.dataset, "n_ions": len(data), "flight_path_mm": fpl, "det_diam_mm": dd, "results": results},
            f,
            indent=2,
            default=str,
        )
